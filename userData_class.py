...
class UserData:
    ...
    def classify_step(self,
        ...
        for userid in user_data.keys():
            neighbors = [i for i in G.neighbors(userid) if i not in [0, 1]] #SYNTAX HERE
            ein = [i for i in neighbors if i in PL] #Printable
            eout = list(set(neighbors) - set(ein)) #printable
            total_neighbor_capacity = sum([G[userid][i]['capacity'] for i in neighbors]) #printable
            if total_neighbor_capacity < prune_capacity_threshold:
                pruned_nodes.append(userid)
            psi_l = sum([G.edges[userid,i]['capacity'] for i in ein]) - sum([G.edges[userid,j]['capacity'] for j in eout])
            #print(psi_l)
        ...
        return len(PL), len(pruned_nodes), G
    ...
    def make_map_ROC(self,
        ...
        if len(all_latlongs) > 0:
            ...
            s.sort()
            ins = len([i for i in gt if i[1]]) #Why is ins 0
