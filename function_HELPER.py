...
def fit_LR(data_obj,
    ...
    if sum(y_train)<3 or sum(y_train)>(len(y_train)-3):
        ...
        for lambd in l_values:
            ...
            LR=LogisticRegression(penalty=penty,C=lambd, solver='liblinear')
        ...
        LR=LogisticRegression(C=lambd,penalty=penty, solver='liblinear')
        LR.fit(x_train,y_train)
...
def psi(left_right,right_left,u1_features,u2_features):
    ...
    if left_right:
        if u1_features['friends_count']==0 or u2_features['followers_count']==0:
            ...
            print(f'u1_features: {u1_features}')
            print(f'u2_features: {u2_features}')
    ...
    if right_left:
        if u1_features['followers_count']==0 or u2_features['friends_count']==0:
            print("Relationship problem: "+str(u1_features['user_id'])+" <-- "+str(u2_features['user_id'])) #Line 171 follower count problem
